...
class TransmissionLine(object):
    ...
    def __init__(self, conf, df_towers, ps_line):
        ...
        # self.df_towers['id_sides'] = self.df_towers.apply(
        #     lambda x: (self.no_towers - 2, -1) if x.name == self.no_towers - 1
        #     else (x.name - 1, x.name + 1), axis=1)

        #     lambda x: (self.no_towers - 2, -1) if x.name == self.no_towers - 1
        #     else (x.name - 1, x.name + 1), axis=1)

        self.towers = dict()
        for _, ps_tower in self.df_towers.iterrows():
            name = ps_tower.Name
            self.towers[name] = Tower(conf=self.conf, ps_tower=ps_tower)

            #print('{}:{}:{}:{}'.format(name, ps_tower.Function,
            #                           self.towers[name].id,
            #                           self.towers[name].ps_tower.id_adj))

            self.towers[name].calculate_cond_pc_adj()
    ...
    def sort_by_location(self):
        ...
        self.df_towers.index = idx_sorted
        self.name_by_line = [self.df_towers.loc[i, 'Name'] for i in
                             range(self.no_towers)]
        self.id2name = self.df_towers['Name'].to_dict()

        self.df_towers['id_adj'] = self.df_towers.apply(
            self.assign_id_adj, axis=1)

        self.df_towers['actual_span'] = self.ps_line['actual_span']
    ...
    def assign_id_adj(self, ps_tower):
        """
        assign id of towers on both sides
        :param id_end: id of last tower (= no. of towers -1)
        :return:
        """

        idx = ps_tower.name
        max_adj = ps_tower.max_no_adj_towers

        list_left = self.create_list_idx(idx, -1, max_adj)
        list_right = self.create_list_idx(idx, 1, max_adj)

        id_adj = list_left[::-1] + [idx] + list_right

        for i, tid in enumerate(id_adj):
            try:
                tower_function = self.df_towers.loc[tid, 'Function']
            except KeyError:
                pass
            else:
                if tower_function in self.conf.strainer:
                    id_adj[i] = -1
        return id_adj

    def create_list_idx(self, idx, flag_direction, no_towers):
        """
        create list of adjacent towers in each direction (flag=+/-1)

        :param id_end: id of last tower (= no. of towers -1)
        :param flag_direction: +/-1
        :return:
        """
        list_id = []
    ...
    #     for i, tid in enumerate(id_adj):
    #         try:
    #             tower_function = self.df_towers.loc[i, 'Function']
    #         except KeyError:
    #             pass
    #         else:
    #             if tower_function in self.conf.strainer:
