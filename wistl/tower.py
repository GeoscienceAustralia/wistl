...
def compute_tower_damage(event, lines, tower, cfg):
    """
    compute probability exceedance of damage of tower in isolation (Pc)
    Note: dmg index is not identical to wind index
    """
    ...
    #print(f'Process {os.getpid()} done processing record {event.name} - {tower.name}')
    ...
    # dmg_time_idx
    dmg_time_idx = set_dmg_time_idx(dmg, wind)

    #dmg_idxmax = set_dmg_idxmax(dmg, wind)
...
def set_dmg_idxmax(dmg, wind):
    """
    return index of max. dmg relative to wind time index
    """
    if not dmg.empty:
        idt = dmg.loc[dmg['collapse']==dmg['collapse'].max()].index
        return [wind.index.get_loc(x) for x in idt]
...
class FrenchDeck2:
    ...
    def __init__(self):
        self._cards = {(suit, rank): Card(rank, suit) for suit in self.suits
