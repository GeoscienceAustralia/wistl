...
class Event(object):
    ...
    def cal_pc_wind(self):
        ...
        - nds:
        """
        pc_wind = np.zeros(shape=(len(self.idx_time), self.tower.conf.nds))

        try:
            fragx = self.tower.conf.frag[self.tower.ttype][self.tower.funct]
            idf = np.sum(fragx['dev_angle'] <= self.tower.dev_angle)

            for (ds, ids) in self.tower.conf.ds_list:  # damage state
        ...
        self.pc_wind = pd.DataFrame(pc_wind, columns=[x[0] for x in self.tower.conf.ds_list], index=self.wind.index)
    ...
    def cal_mc_adj(self, asset, ds_list, rv, idx):
        """
